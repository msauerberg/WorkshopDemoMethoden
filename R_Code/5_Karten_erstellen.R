...
# Wir benoetigen lediglich DE auf Bundeslandebene
shape2 <- filter(shape, LEVL_CODE==1 & CNTR_CODE=="DE")

# Wir wollen unsere Lebenserwartungen hinzufuegen
e0.merge <- select(e0.wide, c("PopName", "X2019_f", "X2019_m"))
# Spalten umbennen
colnames(e0.merge)[c(2:3)] <- c("e0_F", "e0_M")

# Schluessel, damit wir beide Datensaetze zusammenfuegen koennen
e0.merge$LEVL_CODE <- 1
# Das sind die NUTS codes für die Bundeslaender in Deutschland
...
maps_conti.m <- shape3 %>% 
  ...
  scale_fill_viridis_c(option = "B", direction=-1)+
  coord_sf(datum = NA)+
  ggtitle("Maenner, Lebenserwartung bei Geburt")+
  theme_map()+
  theme(legend.position = "bottom")+
  labs(fill = "e0 (in years)")
...
# So aendern wir die Beschriftung in der Grafik
facet.labs <- c("Frauen","Maenner")
