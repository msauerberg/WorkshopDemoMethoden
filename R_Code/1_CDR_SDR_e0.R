...
# setwd(~/WorkshopDemoMethoden/) Beispiel für den R Server
...
round(Ratio_CDR, 2) #Deutlich hoeher in Deutschland

# Die aeltere Altersstruktur in DE koennte einen Einfluss haben
C_DE <- Daten$Nx_Deutschland / sum(Daten$Nx_Deutschland)
C_CH <- Daten$Nx_Schweiz / sum(Daten$Nx_Schweiz)

# Einfacher plot
plot(1:21, C_DE, type="l", col="gold", ylab="Bevoelkerungsanteil",
     xlab="Altersgruppe")
...
# Sterberaten sind übers Alter hinweg hoeher in DE
...
# Wir koennten auch annehmen, beide Laender hätten die Altersstruktur von DE
...
round(Sterbetafel_CH$ex[1], 2) #Hoeher in der Schweiz
...
# Vergleich der Ueberlebenskurven fuer Deutschland und Schweiz, 2019
